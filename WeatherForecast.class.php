...
class WeatherForecast
{
     ...
     */
    // public static function setRequest($city = null, $num_of_days = 1)
    ...
    private function getRequest($type_of_request = 'local')
    {
        ...
        $request = self::$request[$type_of_request] . '?key=' . $key .'&q=' . $city . '&format=xml&num_of_days=' . $num_of_days;
        //echo $request; die();
    ...
    public function getLocalWeather()
    {
        ...
        foreach ($doc->weather as $weather) {
            ...
            $weather_max = (self::$us_metric ? $weather->tempMaxF : $weather->maxtempC) . '&#176';
            $weather_min = (self::$us_metric ? $weather->tempMinF : $weather->mintempC) . '&#176';
